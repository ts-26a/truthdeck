...
export default function Sidebar({columns, setColumns}: {columns: ColumnType[], setColumns: React.Dispatch<React.SetStateAction<ColumnType[]>>}) {
  return (
      ...
      >
        {
            columns.map((col) => {
                switch (col.name) {
                    ...
                    case 'UserStatuses':
                        return <SvgIcon
                        svg={Person} size="35px" className="bg-gray-300"
                        key={"user-" + col.id} />
                }
            })
        }
